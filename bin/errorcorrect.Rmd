...
```{r}
varnames <- c("ConsRead")
names(cons.out) <- paste0(params$project_name, "_", params$barcode, "_", varnames, "_", group_ID, "_", seq_along(cons.out))
```

We now output the errorcorrected reads to a FASTQ file.

```{r}
outfile <- paste0(params$project_name, "_", params$barcode, "_", group_ID, "_corrected_all.fastq")
